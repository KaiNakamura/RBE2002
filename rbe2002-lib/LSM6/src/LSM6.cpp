...
bool LSM6::init(deviceType device, sa0State sa0) {
  ...
  switch (device) {
    case device_DS33:  // only this IMU for now...
      address =
          (sa0 == sa0_high) ? DS33_SA0_HIGH_ADDRESS : DS33_SA0_LOW_ADDRESS;
      break;
    default:
      break;
  }
...
double LSM6::rawGyroToDegrees(double raw) {
  return raw * mdpsPerLSB / 1000.0 / gyroODR;
}

double LSM6::degreesToRawGyro(double degrees) {
  return degrees / mdpsPerLSB * 1000 * gyroODR;
}
