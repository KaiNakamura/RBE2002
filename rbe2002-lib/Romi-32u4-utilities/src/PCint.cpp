...
void attachPCInt(uint8_t pcInt, void (*pcisr)(void)) {
  ...
  cli();
  PCICR =
      (1 << PCIE0);  //enable PC interrupts (not a problem to set repeatedly)
...
ISR(PCINT0_vect) {
  ...
  if ((maskedDeltaB & (1 << PCINT0))) {
    pcISR[PCINT0]();
  }
  if ((maskedDeltaB & (1 << PCINT1))) {
    pcISR[PCINT1]();
  }
  if ((maskedDeltaB & (1 << PCINT2))) {
    pcISR[PCINT2]();
  }
  if ((maskedDeltaB & (1 << PCINT3))) {
    pcISR[PCINT3]();
  }
  if ((maskedDeltaB & (1 << PCINT4))) {
    pcISR[PCINT4]();
  }
  if ((maskedDeltaB & (1 << PCINT5))) {
    pcISR[PCINT5]();
  }
  if ((maskedDeltaB & (1 << PCINT6))) {
    pcISR[PCINT6]();
  }
  if ((maskedDeltaB & (1 << PCINT7))) {
    pcISR[PCINT7]();
  }
